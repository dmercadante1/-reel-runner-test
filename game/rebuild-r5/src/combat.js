...
window.GOTHIC_COMBAT={
 ...
 flash(s){const C=GOTHIC_CONFIG,now=s.clock;if(now<s.flashUntil||s.film<C.film.flashCost||now<s.hurtUntil)return;s.film-=C.film.flashCost;s.lastFilmUse=now;s.flashUntil=now+C.film.flashCooldown;s.invUntil=Math.max(s.invUntil,now+210);s.stats.flashes++;s.recoilUntil=now+180;s.pose('recoil');GOTHIC.audio.play('capture');
 ...
 shoot(s,e,kind,dir){const p=s.add.image(e.x+dir*23,kind==='wave'?311:e.y-54,kind==='wave'?'stone':kind==='bone'?'bone-shot':'magic-bolt').setDepth(24);if(kind==='wave'){p.setCrop(0,0,38,16).setOrigin(0,0);p.setPosition(e.x+dir*20,304);}else p.setScale(1).setTint(kind==='bone'?0xe8ddba:0xc595f5).setBlendMode(Phaser.BlendModes.ADD);
 ...
 const dx=(p.x-px)*dir,half=8+Math.max(0,dx)*.23;if(kind!=='wave'&&s.beaming&&dx>0&&dx<GOTHIC_CONFIG.capture.range&&Math.abs(p.y-py)<half){p.destroy();s.stats.blocked++;continue;}
