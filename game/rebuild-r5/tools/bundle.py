...
worker=(R/'src/service-worker.template.js').read_text().replace('__BUILD__',info['sha256'][:12]);(out/'sw.js').write_text(worker)
subprocess.run(['node','--check',str(out/'sw.js')],check=True,capture_output=True)
