...
def load(b,mobile=False,canvas=False):
 ...
 p=c.new_page();p.set_default_timeout(10000);p.on('pageerror',lambda e:errors.append(str(e)));p.route('**/game/releases/gothic-hud-r2/',lambda route:route.fulfill(body=H.replace('type:Phaser.AUTO','type:Phaser.CANVAS'),content_type='text/html')) if canvas else None;p.goto('http://127.0.0.1:40723/',wait_until='load');p.wait_for_function('GOTHIC.phase==="ready"');return c,p
...
try:
 ...
 data={'build':json.loads((R/'build.json').read_text()),'scope':'webkit hosted exact R2 bytes; physical devices not tested','passed':sum(r['passed']for r in results),'failed':sum(not r['passed']for r in results),'fatal':fatal,'checks':results,'uncaughtErrors':errors,'recordedAt':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())};(R/'evidence/acceptance-results.json').write_text(json.dumps(data,indent=2));print('TOTAL',data['passed'],'passed',data['failed'],'failed','fatal',fatal,flush=True)
