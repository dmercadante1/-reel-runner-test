...
class GothicHorror extends Phaser.Scene{
  ...
  makeAnimations(){
    ...
    TYPES.forEach(type=>{const start=M.rows[type],rate=C.enemies[type].walkRate;this.anims.create({key:type+'-walk',frames:this.anims.generateFrameNumbers('enemyAtlas',{start,end:start+3}),frameRate:rate,repeat:-1})});
  }
  spawn(type,x,y){
    const cfg=C.enemies[type],start=M.rows[type];const e=this.enemies.create(x,y,'enemyAtlas',start).setScale(cfg.scale).setDepth(8);
    ...
    e.body.setSize(type==='monster'||type==='werewolf'?70:54,type==='monster'||type==='werewolf'?108:98).setOffset(34,20);
  ...
  captureEnemy(e,dt){
    const type=e.getData('type'),cfg=C.enemies[type],rate=C.capture.baseRate/cfg.resistance;
    let c=e.getData('capture')+rate*dt;e.setData('capture',c);const dir=this.player.flipX?-1:1;e.x+=dir*-C.capture.pullStrength*dt*.18;
    if(c>=1){this.captured++;this.film=Math.min(C.film.max,this.film+C.film.captureReward);e.destroy();this.updateHud();}
  }
  update(time,dtMs){
    ...
    const speed=canBeam?C.player.captureMoveSpeed:C.player.speed;
    if(left){this.player.setVelocityX(-speed);this.player.setFlipX(true)}else if(right){this.player.setVelocityX(speed);this.player.setFlipX(false)}else this.player.setVelocityX(this.player.body.velocity.x*.72);
    if(Math.abs(this.player.body.velocity.x)>12&&!canBeam)this.player.play('hero-walk',true);else this.player.anims.stop();
    if(jump&&this.player.body.blocked.down&&!this.jumpLatch&&!canBeam){this.player.setVelocityY(C.player.jumpVelocity);this.jumpLatch=true}if(!jump)this.jumpLatch=false;
    ...
    this.beam.clear();this.bars.clear();
    if(canBeam){this.film=Math.max(0,this.film-C.film.drainPerSecond*dt);const dir=this.player.flipX?-1:1,px=this.player.x+dir*38,py=this.player.y-22,range=C.capture.range,spread=C.capture.spread;this.beam.fillStyle(C.capture.beamColor,C.capture.beamAlpha).beginPath().moveTo(px,py).lineTo(px+dir*range,py-spread/2).lineTo(px+dir*range,py+spread/2).closePath().fillPath();this.enemies.children.iterate(e=>{if(!e?.active)return;const dx=(e.x-this.player.x)*dir,dy=Math.abs(e.y-this.player.y);if(dx>0&&dx<range&&dy<spread*.72)this.captureEnemy(e,dt)});}else this.film=Math.min(C.film.max,this.film+C.film.recoverPerSecond*dt);
    this.enemies.children.iterate(e=>{if(!e?.active)return;const c=e.getData('capture')||0;if(c>0){this.bars.fillStyle(0x0b0d13,.85).fillRect(e.x-31,e.y-76,62,7);this.bars.fillStyle(0x9fe8ff,1).fillRect(e.x-29,e.y-74,58*Math.min(1,c),3)}});
    this.updateHud();
  }
