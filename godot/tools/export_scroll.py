...
engine=sys.argv[1]
out=Path(sys.argv[2] if len(sys.argv)>2 else 'build/scroll').resolve()
out.mkdir(parents=True,exist_ok=True)
assert Path('godot/assets/private-character/model.png').exists(), 'Authorized private character source is required'
subprocess.run([engine,'--headless','--path','godot','--export-release','Scrolling Courtyard',str(out/'index.html')],check=True)
...
(out/'manifest.json').write_text(json.dumps({'build':'courtyard-scroll-01','engine':'4.7.2.stable','render':[1920,1080],'world':[640,360],'files':files},indent=2))
